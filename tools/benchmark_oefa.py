...
def make_batch(n, size, device):
    return {
        "img": torch.rand(n, 3, size, size, device=device),
        "batch_idx": torch.arange(n, device=device),
        "cls": torch.zeros(n, 1, device=device),
        "bboxes": torch.tensor([[0.5, 0.5, 0.2, 0.2]], device=device).repeat(n, 1),
    }


def main(a):
    if not torch.cuda.is_available():
        raise SystemExit("CUDA is required for synchronized benchmark results")
    device = torch.device("cuda")
    rows = []
    names = list(MODELS) if a.model == "all" else [a.model]
    for name in names:
        model = DetectionModel(MODELS[name], nc=a.nc, verbose=False).to(device).train()
        model.args = get_cfg()
        model.criterion = model.init_criterion()
        hook_handles = []
        if a.profile:
            from ultralytics.nn.modules import Detect
            from ultralytics.nn.modules.oefa_v2 import (
            ...
            def label(module):
                if isinstance(module, OEFAGuidedSamplerV2):
                    return "P5toP4 sampler" if module.evidence_level == 1 else "P4toP3 sampler"
                if isinstance(module, OEFABoundaryDownsampleV2):
                    return "boundary downsample P3toP4" if module.evidence_level == 0 else "boundary downsample P4toP5"
                if isinstance(module, EvidenceTargetGeneratorV2):
                    return "target generator"
                if isinstance(module, OEFAEvidencePredictorV2):
                    return "evidence predictor"
                if type(module) is Detect:
                    return "Detect"

            for module in (
                (*model.modules(), *model.criterion.target_generator.modules())
                if hasattr(model.criterion, "target_generator")
                else model.modules()
            ):
                tag = label(module)
                if tag:

                    def pre(mod, inp, tag=tag):
                        mod._oefa_profiler_scope = torch.profiler.record_function(tag)
                        mod._oefa_profiler_scope.__enter__()

                    def post(mod, inp, out):
                        mod._oefa_profiler_scope.__exit__(None, None, None)

                    hook_handles.extend((module.register_forward_pre_hook(pre), module.register_forward_hook(post)))
        optimizer = torch.optim.SGD(model.parameters(), lr=1e-3)
        batch = make_batch(a.batch, a.imgsz, device)
        ...
        def step():
            ...
            with (
                ...
                if a.profile
                else torch.autograd.profiler.record_function("backward")
            ):
                scaler.scale(loss).backward()
            optimizer.step() if not a.amp else scaler.step(optimizer)
            scaler.update()
        ...
        if a.profile:
            ...
            for _ in range(a.iters):
                step()
        torch.cuda.synchronize()
        elapsed = (time.perf_counter() - start) * 1000 / a.iters
...
if __name__ == "__main__":
    ...
    p.add_argument("--output", default="oefa_profiles")
    a = p.parse_args()
