...
def _tiny_dataset(root: Path) -> Path:
    for split in ("train", "val"):
        ...
        for i in range(2):
            Image.fromarray(np.full((64, 64, 3), 64 + i * 64, dtype=np.uint8)).save(root / "images" / split / f"{i}.jpg")
...
def test_oefa_v2_one_epoch_reaches_and_finishes_validator(tmp_path):
    result = YOLO(M1).train(data=_tiny_dataset(tmp_path / "data"), epochs=1, imgsz=64, batch=1, workers=0,
                            device="cpu", pretrained=False, val=True, save=False, plots=False,
                            project=tmp_path / "runs", name="validator-smoke", verbose=False)
