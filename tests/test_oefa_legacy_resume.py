...
def test_legacy_yamls_build_and_forward():
    for yaml in (IDENTITY, M1):
        ...
        with torch.no_grad(): model(torch.zeros(1,3,64,64))
...
def check_checkpoint(weights: str, data: str | None = None, run_val: bool = False):
    ...
    if run_val:
        if not data: raise ValueError("--data is required with --val")
...
if __name__ == "__main__":
    p=argparse.ArgumentParser(); p.add_argument("--weights", required=True); p.add_argument("--data"); p.add_argument("--val", action="store_true")
    a=p.parse_args(); check_checkpoint(a.weights, a.data, a.val)
